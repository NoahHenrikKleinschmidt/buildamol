...
def make_graph(scaffold):
    graph = scaffold.make_atom_graph()
    glycans = scaffold.find_glycans()
    rotatable_edges = []
    circulars = graph.nodes_in_cycles
    for anchor, g in glycans.items():
        rotatable_edges.append((anchor, g.root_atom))
        rotatable_edges.extend(
            bond
            for bond in g.get_bonds()
            if bond not in circulars and len(graph.get_descendants(*bond)) > 1
        )
    rotatable_edges = [
        i for i in graph.edges for e in rotatable_edges if i == e or i == e[::-1]
    ]
...
# rotatron parameters
rotatron_params = {"radius": 20}

# the agent function to use
agent = agents.newton_like_optimize

# agent parameters
agent_params = dict(spread=0.8, shake_up_prob=0.2)
