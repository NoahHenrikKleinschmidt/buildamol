...
def make_graph(scaffold):
    ...
    rotatable_edges = []
    circulars = scaffold._AtomGraph.nodes_in_cycles
    rotatable_edges.extend(
        bond
        for bond in graph.edges
        if isinstance(bond[0], bam.core.base_classes.Atom)
        and isinstance(bond[1], bam.core.base_classes.Atom)
        if bond not in circulars and len(graph.get_descendants(*bond)) > 5
    )
    rotatable_edges = [
        i if i == e else i[::-1] if i == e[::-1] else None
        for i in graph.edges
        for e in rotatable_edges
    ]
    rotatable_edges = [i for i in rotatable_edges if i is not None]
