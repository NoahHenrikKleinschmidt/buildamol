...
"""
NOTE
This version of the script is only for scaffold tests...
"""
...
def make_graph(scaffold):
    graph = scaffold.make_residue_graph()
    graph.make_detailed(n_samples=0.7)
    glycans = scaffold.find_glycans()
    rotatable_edges = []
    for anchor, g in glycans.items():
        g_graph = g.make_residue_graph()
        g_graph.make_detailed()
        g_graph.add_edge(g.root_atom, g.root_residue)
        g_graph.add_edge(anchor, g.root_atom)
        rotatable_edges.extend(
            # the other one has convinced me more
            # g_graph.find_rotatable_edges(anchor, min_descendants=35)
            # this one was quite promising!
            g_graph.direct_edges(anchor, g.get_residue_connections(g.root_residue))
        )
        rotatable_edges.append((anchor, g.root_atom))
...
def concatenation_function(self, x):
    ...
    return np.mean(x) + self.pushback * smallest**2


# rotatron parameters
rotatron_params = {
    "radius": 15,
    "crop_nodes_further_than": 2,
    "pushback": 10,
    "concatenation_function": concatenation_function,
}

# the agent function to use
agent = optimizers.scipy_optimize
...
structures_to_run_on = (gl.Scaffold.from_pdb(s) for s in structures_to_run_on)
...
for structure in structures_to_run_on:
    ...
    for r in range(re_runs):
        ...
        if visualize_final_structure:
            ...
            clash_history[structure.id].append(
                auxiliary.compute_clashes(env.state, clash_cutoff)
            )
        if export_pdb:
            sol = auxiliary.apply_solution(sol, env, structure.copy())
            sol.to_pdb(f"{structure.id}_{r}.pdb")

        env.reset()
        print(f"Run {r+1}/{re_runs} done")
    ...
    if visualize_final_structure:
        ...
        v = None
        if export_pdb:
            _best.to_pdb(f"{structure.id}_best.pdb")
...
if visualize_final_structure and export_visualizations:
