...
def make_graph(scaffold):
    graph = scaffold._AtomGraph
    glycans = scaffold.find_glycans()
    rotatable_edges = []
    circulars = []
    for i in graph.nodes_in_cycles:
        circulars.extend(i)

    for anchor, g in glycans.items():
        _graph = nx.dfs_tree(graph, g.root_atom)
        rotatable_edges.append((anchor, g.root_atom))
        rotatable_edges.extend(
            bond
            for bond in _graph.edges
            if not (bond[0] in circulars and bond[1] in circulars)
            and len(graph.get_descendants(*bond)) > 3
        )
